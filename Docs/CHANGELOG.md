...
- **`_build_error_payload` renamed to `build_error_payload`.** It is defined in `pulseengine/core/errors.py` but consumed from `pulseengine/local/scan.py` as well as `core/app.py` — a name used by another package is not private, and the underscore was telling readers the opposite.
- **`pandas-stubs` added to the dev toolchain, pinned to match the pandas pin.** Without stubs every DataFrame expression is `Any`, so type errors are hidden rather than reported. The version must track `requirements.txt` (pandas 2.3.3 → pandas-stubs 2.3.3.*): stubs for a different pandas major produce "incompatible stub package" warnings and type errors describing an API the project does not run.
- **`[project.optional-dependencies] dev` added.** Tooling reads project metadata, not loose requirements files, so with `pytest` living only in `requirements-dev.txt` every `import pytest` in `tests/` was flagged as an undeclared dependency. `pip install -e ".[dev]"` now gets the full toolchain.
- **Module-level script variables in `pulseengine/web/dashboard.py` prefixed with `_`.** Sixteen of them shadowed identically-named locals inside `_build_live_analysis`, so the same identifier meant two different things depending on where you read it. Now matches the convention already used in `pulseengine/local/dashboard.py`.
- `install.py` no longer wraps `sys.stdout.reconfigure` in `except AttributeError`; it checks with `getattr` instead, which is type-clean and cannot mask an unrelated `AttributeError` raised from inside the call.
